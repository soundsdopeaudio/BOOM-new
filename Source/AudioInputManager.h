...
private:
    ...
    // IMPORTANT: exact signature must match JUCE's AudioIODeviceCallback for your JUCE version.
    // Many JUCE versions declare:
    // `virtual void audioDeviceIOCallback (const float** inputChannelData, int numInputChannels,
    //                                       float** outputChannelData, int numOutputChannels,
    //                                       int numSamples) = 0;`
    // Use the signature below to match that form exactly (no extra inner-const qualifiers).
