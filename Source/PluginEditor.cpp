...
void BoomAudioProcessorEditor::performFileDrag(const juce::File& midiFile)
{
    ...
    // Try to find the nearest DragAndDropContainer to perform an external OS drag
    Component* comp = this;
    juce::DragAndDropContainer* dnd = nullptr;

    // walk up parent chain defensively (findParentDragContainerFor sometimes returns null)
    ...
    if (dnd != nullptr)
    {
        ...
        DBG("performFileDrag: started external drag for " << midiFile.getFullPathName());
    }
    else
    {
        // Graceful fallback: show the file location so the user can find it manually
        DBG("performFileDrag: no DragAndDropContainer found for editor - cannot start external drag.");
        juce::AlertWindow::showMessageBoxAsync(juce::AlertWindow::WarningIcon,
            "Drag not available",
            "Couldn't start a drag operation in this context.\nTemporary MIDI file was saved to:\n\n" + midiFile.getFullPathName());
    }
}
...
// Update range from actual generated notes
    ...
    dragAll.onClick = [this]()
        {
            ...
            // Build full-MIDI and get temp file
            juce::File f = writeTempMidiFile();
            if (!f.existsAsFile())
            {
                DBG("UI: dragAll -> temp MIDI not found: " << f.getFullPathName());
            }
            else
            {
                // Use the parent DragAndDropContainer (safer across hosts/windows)
                if (auto* dnd = juce::DragAndDropContainer::findParentDragContainerFor(this))
                {
                    juce::StringArray files; files.add(f.getFullPathName());
                    dnd->performExternalDragDropOfFiles(files, true);
                    DBG("UI: dragAll -> started external drag for " << f.getFileName());
                }
                else
                {
                    DBG("UI: dragAll -> no DragAndDropContainer found!");
                }
            }
    ...
    dragSelected.onClick = [this]()
        {
            // read bitmask from the drum grid (1 bit per row)
            const uint32_t rowMask = drumGrid.getRowSelectionMask();
            DBG("dragSelected clicked, rowMask = " + juce::String((int)rowMask));

            if (rowMask == 0)
            {
                // nothing selected -> prompt user and return
                juce::AlertWindow::showMessageBoxAsync(juce::AlertWindow::WarningIcon,
                    "No rows selected",
                    "Please select one or more drum rows before dragging.");
                return;
            }

            // get the processor's drum pattern (const copy)
            const auto pat = proc.getDrumPattern();

            juce::MidiMessageSequence seq;

            // Map row index -> GM drum note (you can adjust these if you have different mapping)
            // 0 = Kick, 1 = Snare, 2 = Closed HH, 3 = Open HH, 4 = Perc1, 5 = Perc2, ...
            auto rowToMidiNote = [](int row)->int
                {
                    switch (row)
                    {
                    case 0: return 36; // Kick (C2)
                    case 1: return 38; // Acoustic Snare
                    case 2: return 42; // Closed Hi-Hat
                    case 3: return 46; // Open Hi-Hat
                    case 4: return 49; // Crash/ride-ish perc
                    case 5: return 51; // Ride bell / another perc
                    default: return 36; // fallback -> kick
                    }
                };

            // Walk pattern and only include events where the row bit is set
            for (int i = 0; i < pat.size(); ++i)
            {
                const auto& n = pat.getReference(i);
                const uint32_t bit = (1u << n.row);
                if ((rowMask & bit) == 0) continue; // this row not selected -> skip

                const int midiNote = rowToMidiNote(n.row);
                const int channel = 10; // drums channel (GM) - adjust if you want different
                const uint8_t vel = (uint8_t)juce::jlimit(1, 127, n.velocity);

                // Add note on / note off at ticks given by startTick/lengthTicks
                seq.addEvent(juce::MidiMessage::noteOn(channel, midiNote, vel), n.startTick);
                seq.addEvent(juce::MidiMessage::noteOff(channel, midiNote), n.startTick + juce::jmax(3, n.lengthTicks));
            }

            // If user selected rows but there were no matching notes (edge-case), show feedback
            if (seq.getNumEvents() == 0)
            {
                juce::AlertWindow::showMessageBoxAsync(juce::AlertWindow::WarningIcon,
                    "No MIDI generated",
                    "The selected rows contain no notes in the current pattern.");
                // restore UI state so drag buttons go away and main drag button comes back
                drumDragChoicesVisible = false;
                dragSelected.setVisible(false);
                dragAll.setVisible(false);
                btnDragMidi.setVisible(true);
            ...
            // Add a tempo meta event so standalone contexts read nicely (optional, matches other code)
            seq.addEvent(juce::MidiMessage::tempoMetaEvent(0.5), 0);
            seq.updateMatchedPairs();

            // Build a MidiFile and write to a temp .mid
            juce::MidiFile mf;
            mf.setTicksPerQuarterNote(BoomAudioProcessor::PPQ); // use processor PPQ constant. :contentReference[oaicite:2]{index=2}
            mf.addTrack(seq);

            juce::File tmp = juce::File::getSpecialLocation(juce::File::tempDirectory)
                .getChildFile("boom_selected_rows.mid");
            ...
            if (!fos.openedOk())
            {
                DBG("dragSelected: failed to open temp file for writing: " + tmp.getFullPathName());
                juce::AlertWindow::showMessageBoxAsync(juce::AlertWindow::WarningIcon,
                    "File error", "Couldn't write temporary MIDI file for dragging.");
                return;
            }
            ...
            // Do the actual OS drag (you already have performFileDrag implemented in the editor)
            performFileDrag(tmp);

            // restore UI state: hide the two small drag buttons, show main drag button
            drumDragChoicesVisible = false;
            dragSelected.setVisible(false);
            dragAll.setVisible(false);
            btnDragMidi.setVisible(true);
