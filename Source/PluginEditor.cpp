...
juce::File BoomAudioProcessorEditor::buildTempMidiForSelectedRows(uint32_t rowMask, const juce::String& baseFileName) const
{
    // Try the grid helper first (exports rows in mask, baseMidi=60 -> C3)
    juce::File f = drumGrid.exportSelectedRowsToMidiTemp(rowMask, baseFileName, 60);

    // Fallback: if grid returned empty file (e.g., write failed), fall back to editor's full export
    if (!f.existsAsFile())
        f = writeTempMidiFile();
...
juce::File BoomAudioProcessorEditor::writeTempMidiFile() const
{
    ...
    else
    {
        for (const auto& n : pattern)
        {
            if (!selectedRows.contains(n.row)) continue;
            const int pitch = juce::jlimit(0, 127, baseMidi + n.row);
    ...
    juce::MidiFile midiFile;
    midiFile.setTicksPerQuarterNote(96); // MUST match internal PPQ (kPPQ_)
