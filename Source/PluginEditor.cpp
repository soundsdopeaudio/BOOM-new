    ...
    drumGrid(p), pianoRoll(p)
{
    ...
    // style ghxstIntensity with app LNF
    ...
    // ----- BounceSync (Abbrev combo) -----
    addAndMakeVisible(bounceSyncStrength);

    // Add full choices as drop-down items (these are the full words the user sees in the popup)
    bounceSyncStrength.addItem("Light", 1);
    bounceSyncStrength.addItem("Medium", 2);
    bounceSyncStrength.addItem("Hard", 3);

    // Attach to APVTS (same as other combos)
    bounceSyncStrengthAtt = std::make_unique<juce::AudioProcessorValueTreeState::ComboBoxAttachment>(proc.apvts, "bouncesync_strength", bounceSyncStrength);

    // After attachment, ensure visible text uses abbreviation for current value
    // If the attachment sets the selectedId, call updateDisplayToAbbrev to convert visible text to the abbrev.

    // Keep consistent look & feel: justification / styling as other combos
    ...
    // Add onChange so when user chooses a different item we set the closed-box to the abbreviation
    bounceSyncStrength.onChange = [this]()
        {
            // ensure APVTS is notified if you aren't using attachments for that
    ...
    // style tripFlipDensity with app LNF

    // Attach
    tripFlipModeAtt = std::make_unique<juce::AudioProcessorValueTreeState::ComboBoxAttachment>(proc.apvts, "mode_TripFlip", tripFlipModeBox);

    // Abbrev display after attachment

    // Ensure density slider is enabled only when not Off (if you have that logic)
    tripFlipModeBox.onChange = [this]()
        {
            const int sel = tripFlipModeBox.getSelectedId();
            tripFlipDensity.setEnabled(sel > 1);
            if (auto* p = proc.apvts.getParameter("mode_TripFlip"))
                p->setValueNotifyingHost((float)(sel - 1));
        };
    ...
    polyGodRatioAtt = std::make_unique<juce::AudioProcessorValueTreeState::ComboBoxAttachment>(proc.apvts, "polygod_ratio", polyGodRatioBox);
    ...
    bounceSyncToggle.onClick = [this]()
        {
            ...
            // ensure slider/combo enabled state reflect toggle
            bounceSyncStrength.setEnabled(newState);
            // APVTS already bound by attachment so no need to write to param explicitly,
            // but if you want to set the parameter from UI directly:
            // proc.apvts.getParameter("mode_BounceSync")->setValueNotifyingHost(newState ? 1.0f : 0.0f);
        };
    ...
    // TripFlip: no exclusive mutual-state needed, but ensure density slider enabled only when not "Off"
    tripFlipModeBox.onChange = [this]()
        {
            const int idx = tripFlipModeBox.getSelectedId() - 1; // ComboBox IDs start at 1
            const bool enabled = (idx > 0);
            tripFlipDensity.setEnabled(enabled);
            if (auto* p = proc.apvts.getParameter("mode_TripFlip"))
                p->setValueNotifyingHost((float)idx);
        };

    // PolyGod toggle: enable/disable ratio box
    ...
    // Scatter button: toggle on/off and open Scatter window (modal)
    scatterBtn.onClick = [this]()
        {
            ...
            // open/close scatterWindow (follow same window pattern as HatsWindow/RollsWindow)
            if (v)
            {
                if (!scatterWindow)
                {
                    scatterWindow = std::make_unique<ScatterWindow>(proc, [this]() // onClose lambda
                        {
                            scatterBtn.setToggleState(false, juce::dontSendNotification);
                            if (auto* pp = proc.apvts.getParameter("mode_Scatter")) pp->setValueNotifyingHost(0.0f);
                            scatterWindow.reset();
                        });
                    scatterWindow->addToDesktop(juce::ComponentPeer::windowAppearsOnTaskbar | juce::ComponentPeer::windowIsTemporary, nullptr);
                }
            }
            else
            {
                if (scatterWindow)
                {
                    scatterWindow->exitModalState(0);
                    scatterWindow.reset();
                }
            }
        };
    ...
    bounceSyncToggleAtt = std::make_unique<juce::AudioProcessorValueTreeState::ButtonAttachment>(proc.apvts, "mode_BounceSync", bounceSyncToggle);
    bounceSyncStrengthAtt = std::make_unique<juce::AudioProcessorValueTreeState::ComboBoxAttachment>(proc.apvts, "bouncesync_strength", bounceSyncStrength);
    ...
    // Prevent the viewport from capturing drags that originate inside the grid so
    // in-component drag operations (note-resize/paint-sweep) work normally.
    drumGrid.setViewportIgnoreDragFlag(true);
    ...
    // Prevent viewport drag capture for the piano roll so resizing notes is possible.
    pianoRoll.setViewportIgnoreDragFlag(true);
    ...
    btnGenerate.onClick = [this]
        {
            ...
            if (eng == boom::Engine::Drums)
            {
                ...
                // ---- Call database generator (PASS effective gated values)
                boom::drums::DrumStyleSpec spec = boom::drums::getSpec(style);
                boom::drums::DrumPattern pat;
                boom::drums::generate(spec, bars, restPct, effectiveDottedPct, effectiveTripletPct, swingPct, /*seed*/ -1, pat);
    ...
    juce::MessageManager::callAsync([this, parameterID, newValue]()
        {
            ...
            else if (parameterID == "mode_GHXSTGRID")
            {
                const bool v = proc.apvts.getRawParameterValue("mode_GHXSTGRID")->load() > 0.5f;
                ghxstToggle.setToggleState(v, juce::dontSendNotification);
                ghxstIntensity.setEnabled(v);
            }
            else if (parameterID == "ghxst_intensity")
            {
                // slider is bound via attachment; no UI update required unless you want to display numeric
            }

            if (parameterID == "mode_BounceSync")
            {
                const bool v = proc.apvts.getRawParameterValue("mode_BounceSync")->load() > 0.5f;
                bounceSyncToggle.setToggleState(v, juce::dontSendNotification);
                bounceSyncStrength.setEnabled(v);
            }
            else if (parameterID == "bouncesync_strength")
            {
                const int idx = juce::jmax(0, (int)juce::roundToInt(proc.apvts.getRawParameterValue("bouncesync_strength")->load()));
                // ComboBox item ids use 1-based IDs
                bounceSyncStrength.setSelectedId(idx + 1, juce::dontSendNotification);

                // Update the closed-box visible text to the abbreviation (LITE/MED/HARD)
            }
            // If other params matter, fall through (no-op)
            else if (parameterID == "mode_NegSpace")
            {
                const bool v = proc.apvts.getRawParameterValue("mode_NegSpace")->load() > 0.5f;
                negSpaceToggle.setToggleState(v, juce::dontSendNotification);
                negSpaceGapSlider.setEnabled(v);
            }
            else if (parameterID == "negspace_gapPct")
            {
                // slider bound by attachment; no manual sync needed
            }
            else if (parameterID == "mode_TripFlip")
            {
                ...
                // Set closed-box to abbreviation (OFF/LITE/MED/AGG)

                // Also enable/disable density slider if you use that UX
                tripFlipDensity.setEnabled((idx + 1) > 1);
            }
            ...
            else if (parameterID == "tripflip_density")
            {
                // slider bound
            }
            else if (parameterID == "mode_PolyGod")
            {
                const bool v = proc.apvts.getRawParameterValue("mode_PolyGod")->load() > 0.5f;
                polyGodToggle.setToggleState(v, juce::dontSendNotification);
                polyGodRatioBox.setEnabled(v);
            }
            else if (parameterID == "polygod_ratio")
            {
                const int idx = juce::jmax(0, (int)juce::roundToInt(proc.apvts.getRawParameterValue("polygod_ratio")->load()));
                polyGodRatioBox.setSelectedId(idx + 1, juce::dontSendNotification);
            }
            else if (parameterID == "mode_Scatter")
            {
...
void BoomAudioProcessorEditor::resized()
{
    ...
    polyGodRatioBox.setBounds(S(400, 322, 60, 20));

    // tripflip (between polygod and bumppit)
    tripFlipLblImg.setBounds(S(460, 310, 80, 20));
    tripFlipToggle.setBounds(S(530, 310, 20, 20));
    tripFlipModeBox.setBounds(S(555, 310, 75, 20));
    tripFlipDensity.setBounds(S(460, 335, 100, 20));

    // Scatter (right-side action area or wherever you put other modal buttons) // example: place near bottom-left
    scatterBtn.setBounds(S(455, 365, 180, 45));
...
void BoomAudioProcessorEditor::mouseDown(const juce::MouseEvent& e)
{
    if (e.eventComponent == &btnDragMidi || e.originalComponent == &btnDragMidi)
    {
        ...
        // start the external drag for the default single-drag behaviour (this is intended)
...
juce::File BoomAudioProcessorEditor::buildTempMidiForSelectedRows(const juce::String& baseName) const
{
    ...
    // Build MidiFile using your project-wide helper (keep PPQ = 96 to match processor/editor).
    juce::MidiFile mf = boom::midi::buildMidiFromDrums(mp, 96);

    // Create a non-conflicting temp filename and write it.
    auto tmp = juce::File::getSpecialLocation(juce::File::tempDirectory)
        .getNonexistentChildFile(baseName, ".mid");

    bool wroteOk = boom::midi::writeMidiToFile(mf, tmp);
    if (!wroteOk || !tmp.existsAsFile())
    {
        DBG("buildTempMidiForSelectedRows: FAILED to write temp midi " << tmp.getFullPathName());
    }
    else
    {
        DBG("buildTempMidiForSelectedRows: wrote " << tmp.getFullPathName() << " size=" << tmp.getSize());
    }
...
juce::File BoomAudioProcessorEditor::buildTempMidiForSelectedRows(uint32_t rowMask, const juce::String& baseName) const
{
    ...
    if (rowMask == 0u)
    {
        return buildBatchDrumMidi(baseName,
            boom::drums::getSpec(drumStyleBox.getText().trim()),
            getBarsFromUI(),
            1,   // howMany
    ...
    juce::Array<int> rowToPitch;
    {
        const auto& rows = proc.getDrumRows(); // returns juce::StringArray
        rowToPitch.ensureStorageAllocated(rows.size());
        for (int i = 0; i < rows.size(); ++i)
            rowToPitch.add(35 + i); // fallback mapping, replace if you have a specific pitch map
    }

    for (const auto& e : mp)
    {
        const int pitch = (rowToPitch.size() > e.row) ? rowToPitch[e.row] : (35 + e.row);
        ...
        const juce::int32 offTick = (juce::int32)(e.startTick + e.lengthTicks);
...
juce::File BoomAudioProcessorEditor::writeTempMidiFile() const
{
    ...
    const juce::File outFile = tempDir.getChildFile("boom_temp_drum_export.mid");
    ...
    auto sanitizePatternForExport = [this](const BoomAudioProcessor::Pattern& src)->BoomAudioProcessor::Pattern
    {
        ...
        // Read density sliders (0..1.0 from APVTS raw values) and convert to 0..100 ints
        int tripletPct = 0;
        int dottedPct = 0;
        ...
        // If toggle off, zero-out densities
        ...
        // helper to find nearest base subdivision ticks
        auto findNearestBaseTicks = [&](int lenTicks)->int
        {
            const int denoms[] = { 1,2,4,8,16,32,64 };
            int best = boom::grid::ticksForDenominator(ppq, 16); // default to 16th
        ...
        for (const auto& n : src)
        {
            ...
            // If neither type allowed, snap both start and length to grid/subdivision
            if (tripletPct == 0 && dottedPct == 0)
            {
                nn.lengthTicks = boom::grid::snapTicksToNearestSubdivision(nn.lengthTicks, ppq, false, false);
                nn.startTick = boom::grid::snapTicksToGridStep(nn.startTick, ppq, cellsPerBeat);
                out.add(nn);
                continue;
            }

            // Otherwise, probabilistically convert some notes to triplet/dotted according to densities
            const int totalPct = tripletPct + dottedPct;
            const int roll = (totalPct > 0) ? rnd.nextInt(100) : 100; // 0..99

            if (totalPct > 0 && roll < tripletPct)
            {
                // convert to triplet duration based on nearest base
            ...
            {
                // keep length but snap to nearest allowed musical subdivision if one type disabled
                if (tripletPct == 0)
                    nn.lengthTicks = boom::grid::snapTicksToNearestSubdivision(nn.lengthTicks, ppq, true, false);
                else if (dottedPct == 0)
                    nn.lengthTicks = boom::grid::snapTicksToNearestSubdivision(nn.lengthTicks, ppq, false, true);
                else
                    nn.lengthTicks = juce::jmax(1, nn.lengthTicks);
            }

            // Always snap start to grid step to keep placement sensible
            nn.startTick = boom::grid::snapTicksToGridStep(nn.startTick, ppq, cellsPerBeat);
    ...
    for (const auto& n : exportPattern)
    {
        ...
        const int channel = 10; // drums typically on channel 10
    ...
    // Important: sort & pair noteOn/noteOff before adding to MidiFile
    seq.updateMatchedPairs();
    seq.sort();

    juce::MidiFile midiFile;
    midiFile.setTicksPerQuarterNote(96); // MUST match internal PPQ (kPPQ_)2
...
void BoomAudioProcessorEditor::startExternalMidiDrag()
{
    ...
    if (eng == boom::Engine::Drums)
    {
        // If user has rows selected, use the selected-rows builder so sub-16th notes survive
        const uint32_t rowMask = drumGrid.getRowSelectionMask();
        if (rowMask != 0u)
        {
            tmp = buildTempMidiForSelectedRows(rowMask, "BOOM_SelectedRows");
            DBG("startExternalMidiDrag: drums - using buildTempMidiForSelectedRows rowMask=0x"
                << juce::String::toHexString(rowMask));
        }
        else
        {
            tmp = writeTempMidiFile();
    ...
    btnGenerate.onClick = [this]
        {
            ...
            juce::String keyName = keyBox.getText().trim();
            juce::String scaleName = scaleBox.getText().trim();

            KeyScale ks;
            ks.rootIndex = kKeys.indexOf(keyName.toUpperCase());

            auto it = kScales.find(scaleName);
            ks.pcs = (it != kScales.end()) ? &it->second : &kScales.at("Chromatic");
...
// ------------------- ScatterWindow implementation -------------------
// Place this in PluginEditor.cpp near other Window implementations (HatsWindow / RollsWindow).

ScatterWindow::ScatterWindow(BoomAudioProcessor& p, std::function<void()> onClose)
    : proc(p), onCloseFn(std::move(onClose))
{
    // Size: similar compact window used by Hats/Rolls windows
    setSize(520, 320);

    // Title image centered at top — uses scatterLbl.png as requested
    scatterTitleImg.setImage(loadSkin("scatterLbl.png"));
    addAndMakeVisible(scatterTitleImg);

    // Randomize / dice button (reuse dice skin if available)
    setButtonImages(diceBtn, "diceBtn"); addAndMakeVisible(diceBtn);
    diceBtn.setTooltip("Randomize scatter settings");

    // Primary actions (Generate, Save, Drag)
    setButtonImages(btnGenerate, "generateBtn"); addAndMakeVisible(btnGenerate);
    setButtonImages(btnSave, "saveMidiBtn");     addAndMakeVisible(btnSave);
    setButtonImages(btnDrag, "dragBtn");         addAndMakeVisible(btnDrag);

    // Depth slider
    addAndMakeVisible(depthSlider);
    depthSlider.setSliderStyle(juce::Slider::LinearHorizontal);
    depthSlider.setTextBoxStyle(juce::Slider::NoTextBox, false, 0, 0);
    boomui::makePercentSlider(depthSlider);
    depthSlider.setTooltip("Scatter Depth (binds to main editor scatter depth)");

    // Density combo (Mild / Normal / Spicy)
    addAndMakeVisible(densityBox);
    densityBox.addItemList(juce::StringArray{ "Mild", "Normal", "Spicy" }, 1);
    densityBox.setSelectedId(2, juce::dontSendNotification);

    // Row toggles (Kick, Snare, Hat, OpenHat, Perc1, Perc2, Perc3)
    // Use same toggle art as other checkboxes (checkBoxOffBtn / checkBoxOnBtn)
    const juce::String rowNames[] = { "Kick", "Snare", "Hat", "OpenHat", "Perc1", "Perc2", "Perc3", "Ride" };
    for (int i = 0; i < 8; ++i)
    {
        addAndMakeVisible(rowToggle[i]);
        boomui::setToggleImages(rowToggle[i], "checkBoxOffBtn", "checkBoxOnBtn");
        rowToggle[i].setTooltip("Scatter on " + rowNames[i]);
        // default enable hats/percs
        if (i == 2 || (i >= 4 && i <= 6)) rowToggle[i].setToggleState(true, juce::dontSendNotification);
    }

    // Hook up buttons
    diceBtn.onClick = [this]()
        {
            // quick randomize: random depth and density
            juce::Random r;
            depthSlider.setValue(r.nextInt({ 0, 101 }), juce::dontSendNotification);
            densityBox.setSelectedId(1 + r.nextInt({ 3 }), juce::dontSendNotification);
        };

    btnGenerate.onClick = [this]()
        {
            // When user generates from this window, we will produce a single temp MIDI that includes scatter
            // (scatter generation is handled in the main generator pass by reading APVTS params). To ensure
            // current window params apply, write them into APVTS before invoking proc.setDrumPattern or batch ops.
            if (auto* pDepth = proc.apvts.getParameter("scatter_depth"))
                pDepth->setValueNotifyingHost(depthSlider.getValue() / 100.0f);
            if (auto* pDen = proc.apvts.getParameter("scatter_density"))
                pDen->setValueNotifyingHost((float)(densityBox.getSelectedId() - 1));

            // Kick off generation in the processor: trigger a regenerate (this is consistent with how other windows do it)
            proc.setDrumPattern(proc.getDrumPattern()); // (no-op forcing UI sync) or call a regenerate method if you have one
            TransientMsgComponent::launchCentered(this, "SCATTER APPLIED", 900);
        };

    btnSave.onClick = [this]()
        {
            juce::File tmp = buildTempMidi();
            if (!tmp.existsAsFile()) return;

            auto* fc = new juce::FileChooser("Save Scatter MIDI...", tmp, "*");
            fc->launchAsync(juce::FileBrowserComponent::saveMode,
                [tmp, fc](const juce::FileChooser& chooser)
                {
                    const juce::File dest = chooser.getResult();
...
{
    // nothing special to free here (UI children are owned)
}

juce::File ScatterWindow::buildTempMidi() const
{
    // Build from the current drum pattern (main processor holds the up-to-date pattern).
    // Use the same midi builder helper used by Hats/Rolls windows to keep consistency.
    boom::midi::DrumPattern mp;
    for (const auto& n : proc.getDrumPattern())
        mp.add({ n.row, n.startTick, n.lengthTicks, n.velocity });
    ...
    auto tmp = juce::File::getSpecialLocation(juce::File::tempDirectory)
        .getChildFile("BOOM_Scatter.mid");
    boom::midi::writeMidiToFile(mf, tmp);
    return tmp;
}

void ScatterWindow::performFileDrag(const juce::File& f)
{
    if (!f.existsAsFile()) return;
    if (auto* dnd = juce::DragAndDropContainer::findParentDragContainerFor(this))
    {
...
void ScatterWindow::paint(juce::Graphics& g)
{
    g.fillAll(boomtheme::MainBackground());
}

void ScatterWindow::resized()
{
    auto bounds = getLocalBounds();
    const int W = bounds.getWidth();
    const int H = bounds.getHeight();

    // Title image centered top (scatterLbl.png)
    const int titleW = 280;
    const int titleH = 48;
    scatterTitleImg.setBounds((W - titleW) / 2, 12, titleW, titleH);

    // Top row: dice (left), generate/save/drag (right)
    diceBtn.setBounds(16, 18, 48, 48);

    const int rightX = W - 16 - 48;
    btnDrag.setBounds(rightX - 0, 18, 48, 48);
    btnSave.setBounds(rightX - 56, 18, 48, 48);
    btnGenerate.setBounds(rightX - 112, 18, 48, 48);

    // Depth slider + density combo
    depthSlider.setBounds(26, 84, W - 52, 20);
    densityBox.setBounds(26, 110, 120, 22);

    // Row toggles grid (2 columns)
    const int toggW = 22, toggH = 22;
    const int leftX = 26;
    int y = 150;
    const int colSpacing = 160;
    for (int i = 0; i < 8; ++i)
    {
        const int col = i < 4 ? 0 : 1;
        const int idxInCol = i % 4;
        rowToggle[i].setBounds(leftX + col * colSpacing, y + idxInCol * 32, toggW, toggH);
    }
}
