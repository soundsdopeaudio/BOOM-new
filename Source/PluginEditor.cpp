    ...
    drumGrid(p), pianoRoll(p)
{
    ...
    btnGenerate.onClick = [this]
    {
        ...
        if (eng == boom::Engine::e808) // <-- use your exact enum value for 808
        {
            ...
            int restPct = 0;
            if (auto* p = proc.apvts.getRawParameterValue("restDensity808"))
...
// This expects files: checkBoxOffBtn.png and checkBoxOnBtn.png in BinaryData.
    ...
    if (btnRec1.onClick)
    {
        if (proc.isRhythmimickRecording())
            proc.stopRhythmimickRecording();
        else proc.startRhythmimickRecording();
    }

    // Beatbox Record toggle
    if (btnRec4.onClick)
    {
        if (proc.isBeatboxRecording())
            proc.stopBeatboxRecording();
        else proc.startBeatboxRecording();
    }

    recImgNormal_ = loadSkin("recordBtn.png");
    recImgHover_ = loadSkin("recordBtn_hover.png");
    recImgDown_ = loadSkin("recordBtn_down.png");
...
void AIToolsWindow::timerCallback()
{
    ...
    // --- Record button flashing ---
// --- Flash record buttons while capturing ---
    // Toggle flash every ~500 ms

    // --- Recording flash states ---
    // 1) Slow flash ~2 Hz (toggle every ~500 ms)
    // 1) Slow flash ~2 Hz (toggle every ~500 ms)
    static uint32_t lastFlipMs = 0;
    const uint32_t now = juce::Time::getMillisecondCounter();
    const bool isRec = proc.aiIsCapturing(); // from processor
    if (isRec && now - lastFlipMs >= 500u)
    {
        recFlash_ = !recFlash_;
        lastFlipMs = now;

        // swap the record images you already store
        if (recFlash_)
            forceButtonImage(btnRec1, recImgDown_);   // Rhythmimick record button
        else
            forceButtonImage(btnRec1, recImgHover_);

        if (recFlash_)
            forceButtonImage(btnRec4, recImgDown_);   // Beatbox record button
        else
            forceButtonImage(btnRec4, recImgHover_);
    }
    if (!isRec && recFlash_)
    {
        // ensure we leave the buttons in a non-flashing state
        recFlash_ = false;
        forceButtonImage(btnRec1, recImgNormal_);
        forceButtonImage(btnRec4, recImgNormal_);
    }

    // 2) Update seek bars from processor capture state (you already have this helper)
    updateSeekFromProcessor();  // uses proc.getCaptureLengthSeconds(), etc. :contentReference[oaicite:2]{index=2}

    // 3) Enable/disable transport buttons appropriately
    const bool hasCap = proc.aiHasCapture();
    if (activeTool_ == Tool::Rhythmimick)
    {
        btnPlay1.setEnabled(hasCap);
        btnStop1.setEnabled(hasCap || isRec);
        rhythmSeek.setEnabled(hasCap);
    }
    if (activeTool_ == Tool::Beatbox)
    {
        btnPlay4.setEnabled(hasCap);
        btnStop4.setEnabled(hasCap || isRec);
        beatboxSeek.setEnabled(hasCap);
    }

    // repaint the meters/labels in this window
    repaint();
...
void AIToolsWindow::paint(juce::Graphics& g)
{
    ...
    // Rhythmimick meters
    {
        const float L = juce::jlimit(0.0f, 1.0f, proc.getRhythmimickRMSL());
        const float R = juce::jlimit(0.0f, 1.0f, proc.getRhythmimickRMSR());
        drawMiniMeters(g, metersRhBounds, L, R);
    }

    // Beatbox meters
    {
        const float L = juce::jlimit(0.0f, 1.0f, proc.getBeatboxRMSL());
        const float R = juce::jlimit(0.0f, 1.0f, proc.getBeatboxRMSR());
        drawMiniMeters(g, metersBxBounds, L, R);
    }
}
...
void AIToolsWindow::resized()
{
    ...
    metersRhBounds.setBounds(360, y + 62, 90, 10);
    metersBxBounds.setBounds(360, y + 162, 90, 10);
