...
juce::File BoomAudioProcessorEditor::writeTempMidiFile() const
{
    ...
    const File outFile = tempDir.getChildFile("boom_temp_drum_export.mid");
    ...
    const int drumChannel = 10; // GM drums channel

    // Get pattern from processor (expects notes with .row, .startTick, .lengthTicks, .velocity)
    const auto pattern = proc.getDrumPattern();

    // Get selected rows from the grid (if any)
    juce::Array<int> selectedRows;
    bool anySelected = false;

    // Safe: call drumGrid.getSelectedRows() directly
    // (this requires DrumGridComponent::getSelectedRows() to exist and return 0-based row indices)
    selectedRows = drumGrid.getSelectedRows();
    anySelected = (selectedRows.size() > 0);

    // Debugging output so you can see what's actually being used:
    {
        String s;
        s << "writeTempMidiFile() - anySelected=" << (anySelected ? "true" : "false") << " | selectedRows=[";
        for (int i = 0; i < selectedRows.size(); ++i)
        {
            if (i) s << ",";
            s << selectedRows.getUnchecked(i);
        }
        s << "]";
        DBG(s);
    }

    // Also print the distinct rows that exist in the pattern (quick sanity)
    {
        std::unordered_set<int> rowsInPattern;
        for (const auto& n : pattern)
            rowsInPattern.insert(n.row);

        String s;
        s << "writeTempMidiFile() - rowsInPattern=[";
        bool first = true;
        for (auto r : rowsInPattern)
        {
            if (!first) s << ",";
            s << r;
            first = false;
    ...
    // Build a lookup set of rows we should export
    std::unordered_set<int> rowsToExport;
    if (anySelected)
    {
        for (int i = 0; i < selectedRows.size(); ++i)
            rowsToExport.insert(selectedRows.getUnchecked(i));
    }
    else
    {
        // no selection -> export all rows present in pattern
        for (const auto& n : pattern)
            rowsToExport.insert(n.row);
    }

    // Final debug of rowsToExport
    {
        String s;
        s << "writeTempMidiFile() - rowsToExport=[";
        bool first = true;
        for (auto r : rowsToExport)
        {
            if (!first) s << ",";
            s << r;
            first = false;
    ...
    // Add notes to sequence only if their row is in rowsToExport
    for (const auto& e : pattern)
    {
        if (rowsToExport.find(e.row) == rowsToExport.end())
            continue; // skip non-selected rows

        const int pitch = juce::jlimit(0, 127, baseMidi + e.row);
        const uint8 vel = (uint8)juce::jlimit(1, 127, e.velocity);
        const int startTick = e.startTick;
        const int lenTicks = juce::jmax(1, e.lengthTicks);

        seq.addEvent(MidiMessage::noteOn(drumChannel, pitch, vel), startTick);
        seq.addEvent(MidiMessage::noteOff(drumChannel, pitch), startTick + lenTicks);
    }

    // Create MidiFile and write
    MidiFile midiFile;
    ...
    if (!out.openedOk())
    {
        DBG("writeTempMidiFile: failed to open temp file: " + outFile.getFullPathName());
        return {};
    }
    ...
    DBG("writeTempMidiFile: wrote " + outFile.getFullPathName());
    return outFile;
}

void BoomAudioProcessorEditor::startExternalMidiDrag()
{
    // Build the temp MIDI file and start an external file-drag of that file.
    // Use performExternalDragDropOfFiles(...) from DragAndDropContainer (this class inherits it).
    juce::File temp = writeTempMidiFile();

    if (!temp.existsAsFile())
    {
        DBG("startExternalMidiDrag: temp MIDI file missing: " + temp.getFullPathName());
        return;
    }
    ...
    // 'false' => don't request a move; it's a copy-like drag
