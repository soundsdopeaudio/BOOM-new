    ...
    int denominator)
{
    ...
    for (int i = 0; i < howMany; ++i)
    {
        ...
        for (const auto& e : pat)
        {
            ...
            juce::MidiMessage onMsg = juce::MidiMessage::noteOn(9, e.row, (juce::uint8)e.vel);
            onMsg.setTimeStamp(noteOnTick);
            juce::MidiMessage offMsg = juce::MidiMessage::noteOff(9, e.row);
    ...
    btnRec1.onClick = [this]()
        {
            if (!isRecRh_)
            {
                // start capture (processor handles audio-thread capture)
                proc.aiStartCapture(BoomAudioProcessor::CaptureSource::Loopback);
                isRecRh_ = true;
                isRecBx_ = false;
                startTimerHz(8); // make UI blink/update
...
// Collect keys with JUCE HashMap iterator, sort, then add tracks
    ...
    btnGen3.onClick = [this]
        {
            // read A/B styles from the UI boxes
        // read style names from the UI
            const juce::String a = styleABox.getText();
            const juce::String b = styleBBox.getText();

            // read bars from APVTS (fallback to 4)
            int bars = 4;
            if (auto* p = dynamic_cast<juce::AudioParameterInt*>(proc.apvts.getParameter("bars")))
                bars = p->get();

            // blend slider: left = A, right = B
            const float wA = (float)juce::jlimit(0, 100, (int)juce::roundToInt(blendAB.getValue())) / 100.0f;
            const float wB = 1.0f - wA;

            // generate blended pattern in the processor (this calls setDrumPattern inside)
            proc.aiStyleBlendDrums(a, b, bars, wA, wB);

            // update UI — explicit local updates instead of calling editor->regenerate()
            miniGrid.setPattern(proc.getDrumPattern());
            miniGrid.repaint();


            // finally, repaint this window
            repaint();
