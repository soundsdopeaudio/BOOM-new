...
// Update range from actual generated notes
    ...
    dragSelected.onClick = [this]()
        {
            const bool on = dragSelected.getToggleState();

            if (on)
            {
                const uint32_t rowMask = drumGrid.getRowSelectionMask();
                DBG("UI: dragSelected -> building midi for rows mask=" << (uint32_t)rowMask);

                juce::File f = buildTempMidiForSelectedRows(rowMask);
                if (!f.existsAsFile())
                {
                    DBG("UI: dragSelected -> temp MIDI not found: " << f.getFullPathName());
                }
                else
                {
                    if (auto* dnd = juce::DragAndDropContainer::findParentDragContainerFor(this))
                    {
                        juce::StringArray files; files.add(f.getFullPathName());
                        dnd->performExternalDragDropOfFiles(files, true);
                        DBG("UI: dragSelected -> started external drag for " << f.getFileName());
                    }
                    else
                    {
                        DBG("UI: dragSelected -> no DragAndDropContainer found!");
                    }
                }

                // restore and un-latch
                dragSelected.setToggleState(false, juce::dontSendNotification);
                dragSelected.setVisible(false);
                dragAll.setVisible(false);
                drumDragChoicesVisible = false;
