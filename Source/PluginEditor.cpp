...
// This expects files: checkBoxOffBtn.png and checkBoxOnBtn.png in BinaryData.
    ...
    btnRec1.onClick = [this]() { proc.startRhRecord();  };
    btnStop1.onClick = [this]() { proc.stopRhRecord();   };
    btnPlay1.onClick = [this]() { proc.startRhPlay();    };
...
{
    // Rhythmimick meters from processor:
    const float l = proc.getRhRmsL();
    const float r = proc.getRhRmsR();

    // If you keep smoothed copies locally, update them here.
    // Otherwise, just repaint and let paint() query the processor.

    // If you show a simple text/time readout:
    const int playS = proc.getRhPlayheadSamples();
    const int lenS = proc.getRhLengthSamples();
    const double sr = proc.getRhSampleRate();
    playbackSeconds = sr > 0.0 ? playS / sr : 0.0;
    lengthSeconds = sr > 0.0 ? lenS / sr : 0.0;

    repaint(); // so your paint() can redraw meters/time
}
...
void AIToolsWindow::updateSeekFromProcessor()
{
    ...
    const double len = juce::jmax(0.000001, proc.getRhLengthSeconds());
    const double pos = proc.getRhPlayheadSeconds();
...
void AIToolsWindow::paint(juce::Graphics& g)
{
    ...
    auto drawMeter = [&](juce::Rectangle<float> bounds, float value)
    {
        const float w = bounds.getWidth() * juce::jlimit(0.0f, 1.0f, value * 2.0f); // scale RMS a bit
        g.setColour(juce::Colours::darkgrey);
        g.fillRect(bounds);
        g.setColour(juce::Colours::limegreen);
        g.fillRect(juce::Rectangle<float>(bounds.getX(), bounds.getY(), w, bounds.getHeight()));
        g.setColour(juce::Colours::black);
        g.drawRect(bounds);
    };

    // put these where your AI tools meters live
    drawMeter({ 360.0f, 62.0f, 90.0f, 10.0f }, proc.getRhRmsL());
    drawMeter({ 360.0f, 76.0f, 90.0f, 10.0f }, proc.getRhRmsR());
