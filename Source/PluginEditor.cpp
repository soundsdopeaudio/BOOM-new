...
void AIToolsWindow::paint(juce::Graphics& g)
{
    ...
    auto drawMetersLR = [&](juce::Rectangle<int> bounds, float l, float r) {
        auto meterBounds = bounds.reduced(2);
        drawMeter(g, meterBounds.removeFromTop(meterBounds.getHeight() / 2), l);
        drawMeter(g, meterBounds, r);
    };

    drawMetersLR(metersRhBounds, rhL_, rhR_);
    drawMetersLR(metersBxBounds, bxL_, bxR_);
}
...
void AIToolsWindow::resized()
{
    ...
    // Position meters next to their respective sections
    const int meterX = 10;
    const int meterWidth = 120;
    const int meterHeight = 40;
    const int rh_y = 120 + 85;
    const int bx_y = 120 + 3 * vertical_spacing + 85;

    metersRhBounds.setBounds(meterX, rh_y, meterWidth, meterHeight);
    metersBxBounds.setBounds(meterX, bx_y, meterWidth, meterHeight);
