...
struct NeonSliderLNF : public juce::LookAndFeel_V4
{
        ...
        const juce::Slider::SliderStyle /*style*/, juce::Slider& s) override
    {
        ...
        // Tick marks / grid hint (subtle)
        g.setColour(juce::Colour::fromString("FF2D2E41"));
        for (int i = 1; i < 8; ++i)
        {
            auto lx = inner.getX() + inner.getWidth() * (i / 8.0f);
            g.drawVerticalLine((int)std::round(lx), inner.getY(), inner.getBottom());
        }
