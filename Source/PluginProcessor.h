...
// Returns a vector<int> length = bars*16, values in [0..100] = probability boost.
    ...
    // Storage
    MelPattern  melodicPattern_;
    DrumPattern drumPattern_;

    // Accessors — NOTE the types!
    MelPattern& getMelodicPattern() { return melodicPattern_; }
    const MelPattern& getMelodicPattern() const { return melodicPattern_; }
    ...
    DrumPattern& getDrumPattern() { return drumPattern_; }
    const DrumPattern& getDrumPattern() const { return drumPattern_; }
