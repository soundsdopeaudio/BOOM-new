...
public:
    ...
    {
        int   pitch = 60;   // 0..127
        int   startTick = 0;    // ticks
        int   lengthTicks = 24;   // ticks
        int velocity = 100;  // 1..127
        int channel = 1;    // 1..16
    };

    // Notes
    using DrumNote = BoomAudioProcessor::Note;
    using MelNote = BoomAudioProcessor::MelNote;

    // Patterns
    using DrumPattern = juce::Array<DrumNote>;
    using MelPattern = juce::Array<MelNote>;
    using Pattern = DrumPattern;

    // ==== GEN: high-level generation entry points (engines) ====
// 808 generator entry point (called from UI)

    void generate808(int bars, int octave, int densityPct, bool allowTriplets, bool allowDotted, int seed);
...
// Returns a vector<int> length = bars*16, values in [0..100] = probability boost.
    ...
    void   aiPreviewStart();
    void   aiPreviewStop();
    // Add these to the public section of `class BoomAudioProcessor`
    bool aiHasCapture() const noexcept;
    bool aiIsPreviewing() const noexcept;
    ...
    void setGeneratedPattern(const DrumPattern& p);
    ...
    // Accessors — NOTE the types!
    MelPattern& getMelodicPattern() { return melodicPattern; }
    const MelPattern& getMelodicPattern() const { return melodicPattern; }

    void setMelodicPattern(const MelPattern& p)
    {
        melodicPattern = p;
        notifyPatternChanged();
        if (melodicPatternChangedCallback)
            melodicPatternChangedCallback();
    }

    // Public subscription point so UI can be notified when drumPattern changes.
    std::function<void()> drumPatternChangedCallback;
    // Public subscription for melodic pattern changes (piano roll updates)
    std::function<void()> melodicPatternChangedCallback;

    const Pattern& getDrumPattern() const noexcept { return drumPattern; }
    void setDrumPattern(const Pattern& p)
    {
        drumPattern = p;
        notifyPatternChanged();
        if (drumPatternChangedCallback)
            drumPatternChangedCallback();
    }

    const juce::StringArray& getDrumRows() const { return drumRows; }

    // PluginProcessor.cpp
    boom::Engine getEngineSafe() const
    {
...
// ---- Bumppit (Drums path + Melodic path) ----                                               // DRUMS ONLY
    ...
    juce::MidiMessageSequence generateRolls(const juce::String& style,
        int tsNum, int tsDen, int bars,
        bool allowTriplets, bool allowDotted,
        int seed) const;

    void generateRollBatch(const juce::String& style, int tsNum, int tsDen,
        int bars, int howMany, const juce::File& folder,
        bool allowTriplets, bool allowDotted);

    void generateHiHatBatch(const juce::String& style, int tsNum, int tsDen,
        int bars, int howMany, const juce::File& folder,
        bool allowTriplets, bool allowDotted);

    juce::MidiMessageSequence makeHiHatPattern(const juce::String& style,
        int tsNum, int tsDen, int bars,
        bool allowTriplets, bool allowDotted,
        int seed) const;
    ...
    void aiStartCapture(CaptureSource src);  // <-- added declaration
    void aiStopCapture(CaptureSource src);
    void aiStopCapture();

    // <--- changed: declare only (implementation lives in PluginProcessor.cpp) --->
    bool aiIsCapturing() const noexcept;

    float getRhythmimickRMSL() const noexcept { return rmsRhL_; }
    float getRhythmimickRMSR() const noexcept { return rmsRhR_; }
    float getBeatboxRMSL() const noexcept { return rmsBxL_; }
    float getBeatboxRMSR() const noexcept { return rmsBxR_; }
...
private:
    ...
    MelPattern melodicPattern;
    juce::StringArray drumRows { boom::defaultDrumRows() };

     // helper to generate roll patterns (used by generator code)
    void generateRollPattern(juce::MidiMessageSequence& seq,
    ...
    // --- Added to match implementation in PluginProcessor.cpp ---
    Pattern currentPattern;
    void buildPatternForPreview(); // implemented in .cpp
    ...
    std::atomic<bool> isPreviewing{ false };   // <-- missing semicolon fixed
    std::atomic<int> previewReadPos{ 0 };

    // Analysis helpers
    DrumPattern transcribeAudioToDrums(const float* mono, int numSamples, int bars, int bpm) const;
