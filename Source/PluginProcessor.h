...
// ---- Bumppit (Drums path + Melodic path) ----                                               // DRUMS ONLY
    ...
    bool aiIsCapturing() const { return recRh_ || recBx_; }
    ...
    // ==== Rhythmimick capture/playback state ====
    bool   rh_isRecording_ = false;
    bool   rh_isPlaying_ = false;
    int    rh_wrPos_ = 0;      // write head (samples)
    int    rh_rdPos_ = 0;      // read head (samples)
    int    rh_len_ = 0;      // captured length (samples)
    float  rh_rmsL_ = 0.0f;   // last RMS for meters (L)
    float  rh_rmsR_ = 0.0f;   // last RMS for meters (R)
    juce::AudioBuffer<float> rh_buf_; // interleaved “clip” (we’ll keep it stereo)

    // Call from Editor buttons:
    void startRhRecord();
    void stopRhRecord();
    void startRhPlay();
    void stopRhPlay();

    double getRhLengthSeconds() const noexcept { return rh_len_ / lastSampleRate; }
    double getRhPlayheadSeconds() const noexcept { return rh_rdPos_ / lastSampleRate; }

    // For meters in the editor:
    float getRhRmsL() const noexcept { return rh_rmsL_; }
    float getRhRmsR() const noexcept { return rh_rmsR_; }
    int   getRhPlayheadSamples() const noexcept { return rh_rdPos_; }
    int   getRhLengthSamples()  const noexcept { return rh_len_; }
    double getRhSampleRate()    const noexcept { return getSampleRate(); }
