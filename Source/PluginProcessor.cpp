...
void BoomAudioProcessor::generate808(
    ...
    bool allowDotted)
{
    ...
    static const std::map<juce::String, std::vector<int>, std::less<>> eightScales = {
    {"Major",         {0, 2, 4, 5, 7, 9, 11}},
    {"Natural Minor", {0, 2, 3, 5, 7, 8, 10}},
    {"Harmonic Minor",{0, 2, 3, 5, 7, 8, 11}},
    {"Dorian",        {0, 2, 3, 5, 7, 9, 10}},
    ...
    {"Bebop Major",  {0, 2, 4, 5, 7, 8, 9, 11}},
    {"Bebop Minor", {0, 2, 3, 5, 7, 8, 9, 10}}, };
    const auto& scale = (eightScales.count(scaleName) ? eightScales.at(scaleName) : eightScales.at("Natural Minor"));

    auto wrap12 = [](int v) { v %= 12; if (v < 0) v += 12; return v; };
    auto degreeToPitch = [&](int deg, int oct)
    {
        int pc = scale[(deg % (int)scale.size() + (int)scale.size()) % (int)scale.size()];
        return juce::jlimit(0, 127, oct * 12 + wrap12(keyIndex + pc));
    };
    ...
    // RANDOM SETUP
    // ============================================
    juce::Random rng((int)(juce::Time::getHighResolutionTicks() & 0x7fffffff));
    auto pct = [&](int prob) { return rng.nextInt({ 100 }) < prob; };
    ...
    // --- Rhythmic archetype ---
    enum RhType { Eighths, Quarters, Trip8ths, TripQuarters, Sixteenths };
    RhType rhythmType = Eighths;
    int roll = rng.nextInt({ 100 });
    if (roll < 45) rhythmType = Eighths;
    else if (roll < 75) rhythmType = Quarters;
    else if (roll < 90) rhythmType = Trip8ths;
    else if (roll < 97) rhythmType = TripQuarters;
    else rhythmType = Sixteenths;

    // --- Length density (long vs short) ---
    bool longNotes = pct(55); // 55% long/sustained feel, 45% sparse/short

    // --- Pitch mode (root vs chord roots) ---
    bool rootOnly = pct(85); // 85% chance to stick to root

    // --- Complexity ---
    bool complex = pct(40); // 40% chance for rolls & syncopation

    // ============================================
    // TIMING CONFIG PER RHYTHM TYPE
    // ============================================
    int stepSpacing = 2; // default (8th notes)
    switch (rhythmType)
    {
    case Eighths:      stepSpacing = 2; break;
    case Quarters:     stepSpacing = 4; break;
    case Trip8ths:     stepSpacing = 3; break;
    case TripQuarters: stepSpacing = 6; break;
    case Sixteenths:   stepSpacing = 1; break;
    }

    // ============================================
    // 808 NOTE GENERATION
    // ============================================
    const int baseOct = 2; // typical 808 register
    const int sustainDefault = (longNotes ? stepSpacing * 3 : stepSpacing);

    int degree = 0; // root
    int step = 0;
    while (step < totalSteps)
    {
        // Skip randomly to make rhythmic “holes”
        if (pct(40)) { step += stepSpacing; continue; }

        // Velocity variance
        int vel = 90 + rng.nextInt({ 25 });

        // Occasional chord-based variation
        if (!rootOnly && pct(35))
        {
            int pick = rng.nextInt({ 100 });
            if (pick < 50) degree = 4;  // 5th
            else if (pick < 80) degree = 2; // 2nd/9th
            else degree = 7; // 7th for flavor
        }
        else degree = 0; // root

        // Decide sustain length
        int lenSteps = sustainDefault;
        if (allowDotted && pct(15)) lenSteps = (int)(lenSteps * 1.5);
        if (allowTriplets && pct(10)) lenSteps = juce::jmax(1, lenSteps - 1);

        // Add main note
        mp.add({ degreeToPitch(degree, baseOct), step * tps, lenSteps * tps, vel, 1 });

        // Add rolls or syncopations if complex
        if (complex && pct(25))
        {
            int numRolls = rng.nextInt({ 2, 4 });
            int subDiv = tps / numRolls;
            for (int r = 1; r < numRolls; ++r)
            {
                int t = (step * tps) + (r * subDiv);
                mp.add({ degreeToPitch(degree, baseOct), t, subDiv, vel - 10, 1 });
            }
        }
