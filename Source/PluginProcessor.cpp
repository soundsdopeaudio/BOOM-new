...
{
    // --- Read current base style from APVTS (falls back to "trap") ---
    ...
    // --- Read feel sliders (same IDs you already use elsewhere) ---
    const int restPct = clampInt(getPct(apvts, "restDensity", 0) - 10, 0, 100); // keep slightly tighter
    const int dottedPct = getPct(apvts, "dottedDensity", 0);
    int       tripletPct = getPct(apvts, "tripletDensity", 0);
    const int swingPct = getPct(apvts, "swing", 0);
    if (baseStyle.equalsIgnoreCase("drill")) tripletPct = clampInt(tripletPct + 10, 0, 100);

    // --- Validate style name against your style DB ---
    auto styles = boom::drums::styleNames();
    if (!styles.contains(baseStyle))
        baseStyle = styles[0];

    // --- Generate an embellishment pattern from the style DB ---
    boom::drums::DrumStyleSpec spec = boom::drums::getSpec(baseStyle);
    boom::drums::DrumPattern gen;
    boom::drums::generate(spec, bars, restPct, dottedPct, tripletPct, swingPct, /*seed*/ -1, gen);

    // --- Start with the user's skeleton from the mini-grid (ALWAYS keep it) ---
    BoomAudioProcessor::Pattern skeleton = getDrumPattern(); // miniGrid edits are already writing here
    BoomAudioProcessor::Pattern out;
    out.ensureStorageAllocated(skeleton.size() + gen.size());

    // Helper to key hits by (row,startTick) to avoid duplicates
    auto keyOf = [](int row, int startTick) -> int64_t
        {
            return ((int64_t)row << 32) ^ (int64_t)startTick;
        };

    std::unordered_set<int64_t> occupied; occupied.reserve((size_t)(skeleton.size() * 1.5));

    // 1) Copy skeleton first and remember its occupied cells
    for (const auto& n : skeleton)
    {
        out.add(n);
        occupied.insert(keyOf(n.row, n.startTick));
    }

    // 2) Add generated notes where they don't clash with skeleton cells
    for (const auto& e : gen)
    {
        const int row = e.row;
        const int startTick = e.startTick;
        const int lenTicks = e.lenTicks;
        const int vel = juce::jlimit<int>(1, 127, (int)e.vel);

        if (occupied.find(keyOf(row, startTick)) != occupied.end())
            continue; // respect user skeleton

        BoomAudioProcessor::Note n;
        n.row = row;
        n.startTick = startTick;
        n.lengthTicks = lenTicks;
        n.velocity = vel;
        out.add(n);
        occupied.insert(keyOf(row, startTick));
    }
...
namespace {
    constexpr int kPPQ = 96; // ticks per quarter
    ...
    int seed) const
{
    ...
    const int stepsPerBar = stepsPerBarFromTS(tsNum, tsDen); // 16th-steps
    const int totalSteps = stepsPerBar * juce::jlimit(1, 8, bars);

    // ---- Choose one of 3 “paths” (per your spec) ----
    // 0 = steady pulse majority; 1 = spaced/gappy; 2 = wild/unorthodox
    int path = 0;
    {
        int r = rng.nextInt({ 100 });
        if (r < 45) path = 0;
    ...
    // Decide dominant grid size (per style + spec)
    // 0: 8ths, 1: 16ths, 2: 32nds(rare), 3: 8th-trip (rare), 4: 16th-trip (rare), 5: quarters (semi-rare)
    auto chooseMajority = [&]() -> int {
        int r = rng.nextInt({ 100 });
        bool isDrill = style.equalsIgnoreCase("drill");

        if (isDrill)
        {
            // in drill, triplets as common as straight 8ths/16ths
        ...
        if (r < 35) return 0; // 8ths
        if (r < 70) return 1; // 16ths
        if (r < 82) return 5; // quarters (semi-rare in others)
        if (r < 90) return 3; // 8th trip (rare)
        if (r < 96) return 4; // 16th trip (very rare)
        return 2; // 32nds (very rare)
        };

    int majority = chooseMajority();
    if (!allowTriplets && (majority == 3 || majority == 4)) majority = 0;

    auto majorityTick = [&]() -> int {
        switch (majority)
        {
        case 0: return kT8;
        case 1: return kT16;
    ...
    if (style.equalsIgnoreCase("trap") || style.equalsIgnoreCase("drill")) { vMin = 65; vMax = 105; }
    ...
    // Build onsets across the grid
    // Work in ticks directly for precision
    int barLenTicks = stepsPerBar * kT16;
    int totalTicks = totalSteps * kT16;

    // “roll” helper for trap/drill/rnb
    auto addRoll = [&](int t0, int len, bool fast) {
    ...
    // Iterate bars and lay down material per path
    for (int bar = 0; bar < juce::jlimit(1, 8, bars); ++bar)
    {
        int barStart = bar * barLenTicks;

        // choose a bit of variation each bar
        const bool thisBarTrip = allowTriplets && coin(rng, (majority == 3 || majority == 4) ? 80 : 20);
        const bool gappy = (path == 1);
        const bool wild = (path == 2);

        // pulse spacing
        int stepTick = majorityTick;
        if (gappy) stepTick *= (coin(rng, 60) ? 2 : 3); // bigger gaps
        if (wild) stepTick = juce::jmax(kT16T, stepTick / 2); // potential faster motion

        // walk across bar
        for (int t = barStart; t < barStart + barLenTicks; )
        {
            // dotted extend?
            int len = stepTick;
            if (allowDotted && coin(rng, 12)) len = (int)juce::roundToInt(stepTick * 1.5f);

            // choose closed vs open (rare)
            bool useOpen = coin(rng, wild ? 12 : 6);
            int note = useOpen ? openHat : closedHat;

            // occasional rests in gappy or wild
            if (!(gappy && coin(rng, 35)) && !(wild && coin(rng, 15)))
            {
                int vel = rng.nextInt({ vMax - vMin + 1 }) + vMin;

                // potential rolls for trap/drill/r&b on some hits
                if ((style.equalsIgnoreCase("trap") || style.equalsIgnoreCase("drill") || style.equalsIgnoreCase("r&b"))
                    && coin(rng, (wild ? 50 : 25)))
                {
                    addRoll(t, len, coin(rng, 50));
                }
                else
                {
                    addNote(seq, t, juce::jmax(10, len / 2), note, vel, channel);
                }
            }

            // advance by step, with small humanization
            int hop = stepTick;
            if (wild && coin(rng, 20)) hop += (coin(rng, 50) ? +kT16T : -kT16T); // nudge by tiny triplet step
            t += juce::jmax(kT16T, hop);
        }
    }

    // Embed a default tempo meta (120 BPM) so standalone files feel right
    auto tempoMsg = juce::MidiMessage::tempoMetaEvent(0.5 /* seconds per quarter @ 120BPM */);
    seq.addEvent(tempoMsg, 0);

    seq.updateMatchedPairs(); // finalize noteOn/noteOff pairing
    return seq;
}
