...
// shows the current BPM number next to the label
    ...
    juce::File buildTempMidiForSelectedRowsMultiTrack(uint32_t rowMask, const juce::String& baseFileName) const;
    ...
    std::unique_ptr<moreBoomWindow> moreBoom;
...
private:
    ...
    void syncVisibility();
    void regenerate();
    void startExternalMidiDrag(bool multiTrackStems);
 ...
 // temporary placeholder type
    ...
    juce::ImageButton soundsDopeLbl;
...
private:
	...
	juce::ImageComponent keyLbl, scaleLbl, octaveLbl;
...
class moreBoomWindow : public juce::Component
{
public:
    moreBoomWindow(BoomAudioProcessor& p, std::function<void()> onClose);
	~moreBoomWindow() override; 

    void paint(juce::Graphics& g) override;
    void resized() override;

    // Tooltip window for the window
    std::unique_ptr<juce::TooltipWindow> tooltipWindow;

    // Title label at top
    juce::ImageComponent moreBoomLbl;

    // GHXSTGRID UI
    juce::ImageComponent ghxstGridIconLbl;
    ...
    // BounceSync UI
    juce::ImageComponent bounceSyncIconLbl;
    juce::ImageComponent bounceSyncLblImg;
    juce::ImageButton       bounceSyncToggle;
    juce::ComboBox  bounceSyncStrength;
...
// ---- NegSpace UI ----
    juce::ImageComponent negSpaceIconLbl;
    ...
    juce::ImageComponent scatterIconLbl;
    ...
    // GlitchSwitch: OFF / ON / ON+REGEN
    juce::ImageComponent glitchSwitchIconLbl;
    juce::ImageComponent glitchSwitchLbl;
    juce::ComboBox glitchSwitchModeBox;
    std::unique_ptr<juce::AudioProcessorValueTreeState::ComboBoxAttachment> glitchSwitchModeAtt;

    // Holy Rollie: combo box (OFF / ON / ON+REGEN)
    juce::ImageComponent holyRollieIconLbl;
    juce::ImageComponent holyRollieLbl;
    juce::ComboBox holyRollieModeBox;
    std::unique_ptr<juce::AudioProcessorValueTreeState::ComboBoxAttachment> holyRollieModeAtt;

    // -2 In, Drop Out: checkbox
    juce::ImageComponent twoInDropOutIconLbl;
    juce::ImageComponent twoInDropOutLbl;
    juce::ImageButton twoInDropOutToggle;
    std::unique_ptr<juce::AudioProcessorValueTreeState::ButtonAttachment> twoInDropOutToggleAtt;
