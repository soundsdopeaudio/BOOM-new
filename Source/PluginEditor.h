...
private:
    ...
    bool flashOn_ = false;
    juce::int64 lastFlashMs_ = 0; // for 500ms cadence
    ...
    bool flashRh_ = false;     // Rhythmimick REC flashing flag
    bool flashBx_ = false;     // Beatbox REC flashing flag
    int  flashCounter_ = 0;    // ticks for throttling/pulsing

    // --- AI Tools: meter rectangles (set in resized) ---
    juce::Rectangle<int> metersRhBounds { 360, 62, 90, 10 };
    juce::Rectangle<int> metersBxBounds { 360, 162, 90, 10 };
