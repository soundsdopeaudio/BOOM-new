...
class DrumGridComponent : public juce::Component, public juce::DragAndDropTarget, public juce::SettableTooltipClient
{
public:
        ...
        : proc(p), bars(barsToShow), stepsPerBar(stepsPerBar_)
    {
        ...
        // Default note length: 1 step (will be multiplied by ticksPerStep())
        defaultNoteLengthSteps_ = 1;
        currentNoteLengthSteps_ = defaultNoteLengthSteps_;
    }
    ...
    void setPattern(const BoomAudioProcessor::Pattern& pat)
    {
        ...
        // --- Auto-pick grid resolution so sub-16th hits are actually visible ---
        // cellsPerBeat_ meanings (PPQ=96):
        //   4  -> 16ths  (24 ticks)
        //   8  -> 32nds  (12 ticks)
        //   16 -> 64ths  (6 ticks)
        //   24 -> 64th triplets (4 ticks)
        int g = 0;
        for (const auto& n : pat)
        {
            if (n.startTick > 0)   g = gcdInt(g, n.startTick);
            if (n.lengthTicks > 0) g = gcdInt(g, n.lengthTicks);
        }

        int desiredCellsPerBeat = 4; // default 16ths

        // If we see anything compatible with 4-tick timing, go 24 (64T capable)
        if (g != 0)
        {
            if ((g % 4) == 0)          desiredCellsPerBeat = 24; // allows 64th triplets (4 ticks)
            else if ((g % 6) == 0)     desiredCellsPerBeat = 16; // allows 64ths (6 ticks)
            else if ((g % 12) == 0)    desiredCellsPerBeat = 8;  // allows 32nds (12 ticks)
            else                       desiredCellsPerBeat = 4;  // 16ths
        }

        // Clamp to supported set
        if (desiredCellsPerBeat >= 24) desiredCellsPerBeat = 24;
        else if (desiredCellsPerBeat >= 16) desiredCellsPerBeat = 16;
        else if (desiredCellsPerBeat >= 8)  desiredCellsPerBeat = 8;
        else                                desiredCellsPerBeat = 4;

        // If resolution changes, rebuild storage first
        if (cellsPerBeat_ != desiredCellsPerBeat)
        {
            cellsPerBeat_ = desiredCellsPerBeat;
            resized(); // reallocates cells[][] based on totalSteps()
        }
        ...
        for (const auto& n : pat)
        {
            ...
            // CRITICAL FIX: Use the EXACT startTick from the pattern without rounding
            // This ensures sub-16th notes (triplets, dotted, etc.) display at their precise position
            const int step = n.startTick / ticksPerStepLocal;
            if (step >= 0 && step < TS)
            {
    ...
    juce::File exportSelectionToMidiTemp(const juce::String& baseFileName) const
    {
        ...
        for (const auto& n : pat)
        {
            if (n.row < 0 || n.row >= (int)rowEnabled.size())
                continue;

            if (!rowEnabled[(size_t)n.row])
                continue;
    ...
    void paint(juce::Graphics& g) override
    {
        ...
        // ===== 1) HEADER (single system: optional bar row + beat numbers row) =====
        ...
        // Split header into two rows if bar header is enabled
        const int barRowH = showBarHeader ? (headerH / 2) : 0;
        const int beatRowY = headerTop + barRowH;
        const int beatRowH = headerH - barRowH;

        // Bar lines and beat numbers (bar numbers removed)
        ...
        for (int bar = 0; bar < barsToDisplay_; ++bar)
        {
            ...
            // Beat numbers in the header row
            ...
            for (int beatIdx = 0; beatIdx < beatsPerBar; ++beatIdx)
            {
                ...
                juce::Rectangle<int> beatArea(bx, beatRowY, bw, beatRowH);
                // First number in each bar: show BAR NUMBER (1..barsToDisplay_) and tint it brighter.
                // Others remain normal beat numbers (2..beatsPerBar)
                if (beatIdx == 0)
                {
                    // BAR NUMBER: brighter + bold + subtle outline
                    g.setFont(juce::Font(11.5f, juce::Font::bold));

                    g.setColour(boomtheme::LightAccent().withAlpha(1.0f));
                    g.drawFittedText(juce::String(bar + 1),
                        beatArea.translated(1, 1),
                ...
                {
                    g.setFont(juce::Font(11.0f, juce::Font::plain));
                    g.setColour(boomtheme::LightAccent().withAlpha(0.85f));
        ...
        // Horizontal row separators (slightly clearer than normal grid lines)
        for (int row = 0; row <= R; ++row)
        {
            const float y = r.getY() + row * cellH;

            // light line across the GRID only (not the label band)
            g.setColour(PanelStroke().withAlpha(0.18f));

            // slightly thicker every row boundary so it's easier to read
            const float thick = (row % 1 == 0) ? 1.0f : 0.8f;

            g.drawLine(gridX, y, gridX + gridW, y, thick);
        }
        ...
        for (const auto& note : pattern)
        {
            ...
            // No per-row restriction for ornaments in the drum grid — allow densities to apply to all rows

            // If both densities are zero, snap both start and length to common subdivisions/grid so visuals
            // are tidy when ornaments aren't enabled. If either density is enabled, show the actual generated
            // note lengths/starts (so the visual reflects what the generator actually produced per-row).
            const int tps = ticksPerStep();

            // If a note is finer than the current visible grid (or off-grid), DO NOT snap it,
            // because snapping is what makes 1/32T/1/64 look like big 1/16 blocks.
            const bool isFineOrOffGrid =
                (note.lengthTicks < tps) ||
                (note.startTick % tps != 0) ||
                (note.lengthTicks % tps != 0);

            if (!isFineOrOffGrid && tripletPct == 0 && dottedPct == 0)
            {
                // Only snap when it's already aligned and not a micro-roll
            ...
            {
                // Show as-generated so micro-rolls render correctly
                dispLen = note.lengthTicks;
                dispStart = note.startTick;
            }
            ...
            // Decide decorations: show if the note's stored length/start indicates it,
            // or probabilistically based on density sliders so ornaments can appear on any row.
            bool decorateAsDotted = isDotted;
            bool decorateAsTrip = isTrip;

            // If densities provided, allow decorating notes deterministically but pseudo-randomly
            // so visuals reflect density even when stored lengths are straight. Use a per-note seed
            // to keep decorations stable across repaints.
           

            const bool allowDotVisual = showDotted && decorateAsDotted;
            const bool allowTripVisual = showTriplets && decorateAsTrip;

            if (allowDotVisual || allowTripVisual)
            {
                // Use distinct, more subtle accents for dotted vs triplet so both are visible
                // but not overpowering. Triplets get a soft indigo, dotted a muted green.
                juce::Colour tripAccent = juce::Colour::fromString("FF8A63C6"); // light indigo
                juce::Colour dotAccent = juce::Colour::fromString("FF7CD400");   // existing green
                juce::Colour accent = decorateAsTrip ? tripAccent : dotAccent;

                // left accent stripe (thin)
                const float stripeW = juce::jmin(4.0f, noteRect.getHeight() * 0.20f);
                g.setColour(accent.withAlpha(0.95f));
                g.fillRoundedRectangle(noteRect.withX(noteRect.getX() + 2.0f).withWidth(stripeW), 2.0f);

                // soft glow under note for emphasis (subtle)
                g.setColour(accent.withAlpha(0.10f));
                g.fillRoundedRectangle(noteRect.reduced(-2.0f, -2.0f), 4.0f);

                if (decorateAsDotted && showDotted)
                {
                ...
                else if (decorateAsTrip && showTriplets)
                {
        ...
        if (resizing && resizingPatternIndex >= 0)
        {
            ...
            if (resizingPatternIndex < pat.size())
            {
                ...
                if (!matched)
                {
                    // fallback to showing just the tick count as a number
                    labelTxt = juce::String(len);
                }
    ...
    {
        // Ensure cells are properly sized for the current time signature and bars
        resizeCellsToTotalSteps();
        repaint();
    }
    ...
    void mouseDown(const juce::MouseEvent& e) override
    {
        ...
        // Handle Ctrl/Alt modifiers for note length
        if (e.mods.isCtrlDown())
        {
            // Ctrl: double the note length (up to 8x default)
            currentNoteLengthSteps_ = juce::jmin(defaultNoteLengthSteps_ * 8, currentNoteLengthSteps_ * 2);
            repaint(); // show visual feedback if needed
        }
        else if (e.mods.isAltDown())
        {
            // Alt: reset to default length
            currentNoteLengthSteps_ = defaultNoteLengthSteps_;
            repaint();
        }
        ...
        DBG("DrumGridComponent::mouseDown valid=" << (int)h.valid << " onLabel=" << (int)h.onLabel
            << " row=" << h.row << " step=" << h.step << " noteLengthSteps=" << currentNoteLengthSteps_);
        if (!h.valid) return;

        // Check for resize handle FIRST, but only if clicking on an existing note
        if (!h.onLabel && (size_t)h.row < cells.size() && (size_t)h.step < cells[(size_t)h.row].size() && cells[(size_t)h.row][(size_t)h.step])
        {
            const auto pat = proc.getDrumPattern();
            const int ppq = kProjectPPQ;
        ...
        // Right-click: delete any pattern notes that map to this grid cell (unless clicked on label)
        if (e.mods.isRightButtonDown() && !h.onLabel)
        {
            const int ppq = kProjectPPQ;
            const int tps = boom::grid::ticksPerStepFromPpq(ppq, cellsPerBeat_);
            const auto pat = proc.getDrumPattern();
            BoomAudioProcessor::Pattern newPat;
            newPat.ensureStorageAllocated(pat.size());
            int removed = 0;
            for (const auto& note : pat)
            {
                if (note.row == h.row)
                {
                    const int stepIdx = boom::grid::roundStartTickToStepIndex(note.startTick, tps);
                    if (stepIdx == h.step)
                    {
                        ++removed; continue; // delete this note
                    }
                }
                newPat.add(note);
            }
        ...
        // Handle drag-and-drop for existing notes
        if ((size_t)h.row < cells.size() && (size_t)h.step < cells[(size_t)h.row].size() && cells[(size_t)h.row][(size_t)h.step])
        {
            if (auto* container = juce::DragAndDropContainer::findParentDragContainerFor (this))
            {
                ...
                obj->setProperty("lengthTicks", ticksPerStep() * currentNoteLengthSteps_); // Use current length

                juce::var desc (obj);

                // Simple drag image: a small rounded rectangle representing the note
                const int imgW = juce::jmax(24, cellPixelWidth_ * currentNoteLengthSteps_);
        ...
        // Start paint sweep on this row (normal edit) - use current note length
        dragging = true;
        dragRow = h.row;
    ...
    void mouseDrag(const juce::MouseEvent& e) override
    {
        ...
        if (resizing && resizingPatternIndex >= 0)
        {
            ...
            const int ppq = kProjectPPQ;
            auto readBoolParam = [&](const char* idA, const char* idB) -> bool
                {
                    if (auto* p = proc.apvts.getRawParameterValue(idA)) return p->load() > 0.5f;
                    if (auto* p = proc.apvts.getRawParameterValue(idB)) return p->load() > 0.5f;
                    return false;
                };

            const bool showTriplets = readBoolParam("allowTriplets", "useTriplets");
            const bool showDotted = readBoolParam("allowDotted", "useDotted");
    ...
    void setCell(int row, int step, bool v)
    {
        ...
        // When adding a note (v == true), add it to the pattern with current note length
        if (v)
        {
            auto pat = proc.getDrumPattern();
            const int startTick = step * ticksPerStep();
            const int noteLenTicks = ticksPerStep() * currentNoteLengthSteps_;
            
            // Check if a note already exists at this position
            bool exists = false;
            for (const auto& note : pat)
            {
                if (note.row == row && note.startTick == startTick)
                {
                    exists = true;
                    break;
                }
        ...
        {
            // When removing a note (v == false), remove it from the pattern
            auto pat = proc.getDrumPattern();
            const int startTick = step * ticksPerStep();
            BoomAudioProcessor::Pattern newPat;
            newPat.ensureStorageAllocated(pat.size());
    ...
    // Export selected rows to MIDI with each row on its own track (for stems)
    juce::File exportSelectedRowsToMidiTempMultiTrack(uint32_t rowMask, const juce::String& baseFileName, int baseMidi = 60) const;
...
int defaultNoteLengthSteps_ = 1;  // Default: 1 grid step
int currentNoteLengthSteps_ = 1;   // Current drawing length (affected by Ctrl/Alt)
    void resizeCellsToTotalSteps()
    {
        const int TS = totalSteps();
        if (TS <= 0) return;

        for (auto& row : cells)
            row.resize((size_t)TS, false);
    }
    ...
    int totalSteps() const { return barsToDisplay_ * timeSigNum_ * cellsPerBeat_; }
