...
// Quick export of the **enabled rows only** to a temp MIDI file (pure JUCE)
    ...
    void paint(juce::Graphics& g) override
    {
        ...
        if (showBarHeader)
        {
            const int bars = barsToDisplay_;

            int x = leftMargin_; // use your existing left margin
            ...
            for (int bar = 0; bar < bars; ++bar)
            {
                ...
                g.setColour(boomtheme::HeaderBackground());
                g.drawFittedText(juce::String(bar + 1), { x, 0, w, headerH }, juce::Justification::centred, 1);
