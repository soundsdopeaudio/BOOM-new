...
// Quick export of the **enabled rows only** to a temp MIDI file (pure JUCE)
    ...
    void paint(juce::Graphics& g) override
    {
        ...
        for (int bar = 0; bar < barsToDisplay_; ++bar)
        {
            ...
            // Draw "Bar N" centered across the bar region
            {
                const int leftPx = juce::roundToInt(currentX);
                const int wPx = juce::roundToInt(barPixelWidthF);
                juce::Rectangle<int> textArea(leftPx, headerTop, wPx, headerH);

                g.setColour(LightAccent().withAlpha(0.95f));
                g.setFont(juce::Font(12.0f, juce::Font::bold));
                g.drawFittedText(juce::String(bar + 1), textArea, juce::Justification::centred, 1);
            }
            ...
            for (int beatIdx = 0; beatIdx < beatsPerBar; ++beatIdx)
            {
                ...
                g.drawFittedText(juce::String(beatIdx + 1), beatArea, juce::Justification::centred, 1);
            }
