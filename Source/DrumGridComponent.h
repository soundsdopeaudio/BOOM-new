    ...
    void paint(juce::Graphics& g) override
    {
        ...
        for (int bar = 0; bar < barsToDisplay_; ++bar)
        {
            ...
            for (int beatIdx = 0; beatIdx < beatsPerBar; ++beatIdx)
            {
                ...
                g.drawFittedText(juce::String(beatIdx + 1), beatArea, juce::Justification::centredLeft, 1);
            }
        ...
        for (const auto& note : pattern)
        {
            ...
            {
                // Show the pattern as-generated (don't invent triplet/dotted lengths here). This ensures
                // per-row generation decisions (which already add triplets/dotted via the generator) are
                // faithfully displayed and avoids forcing every note to the same ornament.
