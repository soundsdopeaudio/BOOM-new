...
public:
    ...
    void paint(juce::Graphics& g) override
    {
        ...
        for (const auto& note : pattern)
        {
            ...
            {
                // Convert APVTS densities (0..1) -> 0..100
                int tripPct = tripletPct;
                int dotPct = dottedPct;

                if (tripPct > 0 || dotPct > 0)
                {
                    // deterministic per-note pseudo-random via integer hashing to ensure even distribution
                    const uint32_t a = (uint32_t)note.startTick;
                    const uint32_t b = (uint32_t)note.row;
                    const uint32_t c = (uint32_t)note.lengthTicks;
                    uint32_t hash = a * 1664525u + b * 1013904223u + c * 22695477u;
                    int roll = (int)(hash % 100u);
                    const int total = juce::jlimit(0, 100, tripPct + dotPct);
                    if (tripPct > 0 && roll < tripPct)
                        decorateAsTrip = true;
                    else if (dotPct > 0 && roll < tripPct + dotPct)
